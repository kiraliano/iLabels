...
(function (thisObj) {
    ...
    function apiRequest(endpoint, payload) {
        ...
        try {
            ...
            var tmpPath = Folder.temp.fsName + (isWin ? "\\ilabels_req.json" : "/ilabels_req.json");
            var tmpFile = new File(tmpPath);

            tmpFile.encoding = "UTF-8";
            tmpFile.open("w");
            tmpFile.write(JSON.stringify(payload));
            tmpFile.close();

            var url     = API_BASE + endpoint;
            var curlCmd = isWin
                ? "curl.exe -s --max-time 10 -X POST \"" + url + "\" -H \"Content-Type: application/json\" --data-binary @\"" + tmpPath + "\""
                : "curl -s --max-time 10 -X POST \"" + url + "\" -H \"Content-Type: application/json\" --data-binary @\"" + tmpPath + "\"";
            ...
            if (!response || response === "") {
                result.error = "No response (curl not found or network error)";
                return result;
            }
